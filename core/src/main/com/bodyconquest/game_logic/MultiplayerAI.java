...
public class MultiplayerAI extends Thread {
  ...
  /**
   * Instantiates a new Multiplayer ai.
   *
   * @param game the game
   */
  ...
  /**
   * It looks at the players' units strength, base health and current stockpile of resources and
   * computes an Absolute Strength Index (ASI);
   *
   * @return 0 if the two players' ASIs are similar; 1 if the bottom player's ASI is bigger; 2 is
   *     the top player's ASI is
   */
  private int decideWinner() {
    ...
    CopyOnWriteArrayList<Troop> topUnits = game.getTroops(PlayerType.PLAYER_TOP);

    double strength = 0.0;
    for (Troop troop : topUnits) {
      strength =
          (double) troop.getDamage()
              * ((double) troop.getHealth() / 100)
              / ((double) troop.getCooldown() / 1000);
      topASI += strength;
    }
    ...
    int totalTopResources =
        topResources.getLipids() + topResources.getProteins() + topResources.getSugars();

    topASI += (double) totalTopResources / 5;

    // compute bottom player's ASI

    CopyOnWriteArrayList<Troop> botUnits = game.getTroops(PlayerType.PLAYER_BOTTOM);
    ...
    int totalBotResources =
        botResources.getLipids() + botResources.getProteins() + botResources.getSugars();
   ...
   * @param mostDefended if true, then the function returns the lane with strongest units in it;
   *     else, the least defended
  ...
  /**
   * Based on the details given about this class, this method is called when both players are to be
   * attacked
   */
  ...
  /**
   * Spawn units to defend the weaker player in the game, based on the difference between the
   * players' ASIs
   *
  ...
  private void summonUnit(Lane lane, PlayerType playerType) {
    ...
    Gdx.app.postRunnable(() -> game.spawnUnit(UnitType.WHITE_CELL, lane, playerType, true));
  }
}
